  ...
  for (int row = 0; row < height; row++)
	{
		for (int col = 0; col < width; col++)
		{
			int oldRed = image[col][row].r;
			int oldGreen = image[col][row].g;
			int oldBlue = image[col][row].b;
			
			int value = round((oldRed + oldGreen + oldBlue) / 3.0);
			
			image[col][row].r = value;
			image[col][row].g = value;
			image[col][row].b = value;
		}
	}
}
  ...
  // TODO(student): iterate through 2d image of Pixels and convert them to sepia
	for (int row = 0; row < height; row++)
	{
		for (int col = 0; col < width; col++)
		{
			...
			image[col][row].r = newRed;
			image[col][row].g = newGreen;
			image[col][row].b = newBlue;
		}
	}
}
